...
{
public:
	//¬нутри посетител€ есть метод дл€ работы с каждым обьектом.Ёто недостаток 
...
class AbstractDevice abstract
{
	string Producer;
public:
	virtual void Accept(IVisitor* visitor)abstract;//инициализаци€ посетител€,получает какого-то посетител€.
	string GetProducer()
	{ 
		return Producer;
	}
